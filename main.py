...
async def merge(ctx, category):
	...
	if discord.utils.get(ctx.guild.channels, name=activeCTF.getCTF() + "-archive") is None:
		await ctx.guild.create_text_channel(activeCTF.getCTF() + "-archive", category=discord.utils.get(ctx.guild.categories, name="ARCHIVE"))
		archive_channel = bot.get_channel(discord.utils.get(ctx.guild.channels, name=activeCTF.getCTF() + "-archive").id)
