	...
	async def help(self, ctx, *p):
		...
		if p != (): 
			if p[0].lower() == "admin" or p[0].lower() == "1":
				await sendEmbed(ctx,"Help!", "**Admin**\n" + 
				...
				await sendEmbed(ctx,"Help!", "**Player**\n" + 
			...
			elif p[0].lower() == "cryptography" or p[0].lower() == "crypto" or p[0].lower() == "3":
				await sendEmbed(ctx,"Help!", "**Cryptography**\n" + 
				...
				await sendEmbed(ctx,"Help!", "**Other**\n" + 
			...
			await sendEmbed(ctx,"Help!", "*!ctf help [Category]*\n ```fix\n ⮞ Admin```\n ```fix\n ⮞ Player```\n ```fix\n ⮞ Cryptography```\n ```fix\n ⮞ Other```\n https://s1gm0idctf.github.io/bot ")
	pass
